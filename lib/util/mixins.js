...
module.exports = {
	...
	'getProducerOptions' : function getProducerOptions ( options, prefix ) {
	...
	'getConsumerOptions' : function getConsumerOptions ( options, prefix ) {
		// returns error if no msgType or if invalid
		...
		if ( parts instanceof Error ) {
			...
			var partQueue = options.queue || parts.action;
			var queue     = prefix + '.' + parts.resource + '-' + partQueue;
